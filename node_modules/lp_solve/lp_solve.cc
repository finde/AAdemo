...
void LinearProgram::Init(Handle<Object> exports) {
	...
	NODE_SET_PROTOTYPE_METHOD(tpl, "setVerbose", LinearProgram::SetVerbose);
	NODE_SET_PROTOTYPE_METHOD(tpl, "setUnbounded", LinearProgram::SetUnbounded);
...
NAN_METHOD(LinearProgram::SetVerbose) {
	NanScope();
	LinearProgram* obj = node::ObjectWrap::Unwrap<LinearProgram>(args.This());
	int n = Handle<Number>::Cast(args[0])->Int32Value();
  set_verbose(obj->lp, n);
...
NAN_METHOD(LinearProgram::SetUnbounded) {
	NanScope();
	LinearProgram* obj = node::ObjectWrap::Unwrap<LinearProgram>(args.This());
	int n = Handle<Number>::Cast(args[0])->Int32Value();

  bool ret = set_unbounded(obj->lp, n);
